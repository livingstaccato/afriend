...
def persist_skip(store: RunStore, round_no: int, skipped: SkippedFriend) -> dict[str, Any]:
    ...
    return {
        ...
        "fresh_host_worker": skipped.spec.fresh_host_worker,
        "cli": skipped.spec.cli,
...
def persist_result(
    ...
    row = {
        ...
        "fresh_host_worker": spec.fresh_host_worker,
        "cli": spec.cli,
        "model": spec.model,
...
def recover_result_audit(store: RunStore, round_no: int, spec: FriendSpec) -> dict[str, Any]:
    ...
    if not store.owned_regular_exists(path):
        return {
            ...
            "declared_scope": spec.scope,
            "os_confined": False,
            "external_tools": "unrecorded",
            "external_tool_policy": "unrecorded",
            "external_tool_sources": [],
            "deny_external_tools_argv": [],
            "readonly": False,
            "scope": spec.scope,
