...
| Skill routing | [`skill-routing.puml`](skill-routing.puml) | How the host-session resolver invokes the CLI composer, creates a deterministic, content-bound composite + manifest, and enters the normal run snapshot/resume path before `/afriend` preflight, lifecycle events, and read-only status |
