...
| Skill routing | [`skill-routing.puml`](skill-routing.puml) | How the host-session resolver invokes the CLI composer and creates a deterministic, content-bound composite + manifest; the `review` preflight precedes dispatch, and the normal run then creates the snapshot and frozen artifacts/resume path before lifecycle events and read-only status |
